...
class TypeAnalysis(program: AProgram)(implicit declData: DeclarationData) extends DepthFirstAstVisitor[Null] with Analysis[TypeData] {
  ...
  def visit(node: AstNode, arg: Null): Unit = {
    ...
    node match {
      case program: AProgram => program.mainFunction.args.foreach(arg => unify(arg, TipInt()))
      							unify(program.mainFunction.stmts.ret, TipInt())
      ...
      case alloc: AAlloc => unify(alloc, TipRef(alloc.exp))
      ...
      case call: ACallFuncExpr => 
      	unify(call.targetFun, TipFunction(call.args.map(TipType.ast2typevar(_)), call))
      case _: AReturnStmt =>
      case rec: ARecord =>
